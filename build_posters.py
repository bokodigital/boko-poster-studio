...
FAVICON = (BASE / "favicon_datauri.txt").read_text().strip()
...
out = (HTML.replace("__ENGINE__", ENGINE)
           ...
           .replace("__POSTS__", json.dumps(POSTS)).replace("__WEEK_KEY__", WEEK_KEY).replace("__EMAILSIG__", EMAILSIG).replace("__FAVICON__", FAVICON))
